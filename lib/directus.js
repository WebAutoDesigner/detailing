...
// Different transforms are cached separately so multiple sizes can coexist
function downloadImg(uuid, fallback = '', transforms = '') {
  if (!uuid) return Promise.resolve(fallback);

  ensureDir();

  const cacheKey = transforms ? `${uuid}__${transforms.replace(/[^a-z0-9]/g, '_')}` : uuid;
  const existing = fs.readdirSync(LOCAL_IMG_DIR).find(f => f.startsWith(cacheKey));
  if (existing) return Promise.resolve(`/images/directus/${existing}`);
